...
def train_on_device(obj_names, args):
    ...
    for obj_name in obj_names:
        ...
        dataset = MVTec_Anomaly_Detection(args,obj_name,length=args.numbers)
        ...
        for epoch in range(args.epochs):
            ...
            loss_curve.append(average_epoch_loss)
            scheduler.step()

            auroc,auroc_px,ap_px,pro_px=test(args,obj_name, model_seg)
            sum_metric=auroc+auroc_px+pro_px
            if sum_metric>last_sum:
                torch.save(model_seg.state_dict(), os.path.join(args.save_path, run_name + ".pckl"))
                last_sum=sum_metric

        plt.figure()
        plt.plot(range(1, args.epochs + 1), loss_curve, label="Training Loss")
        plt.xlabel("Epoch")
        plt.ylabel("Loss")
        plt.title("Training Loss Curve")
        plt.legend()
        plt.savefig(os.path.join(args.save_path, f"{run_name}_loss_curve.png"))
...
if __name__=="__main__":
    ...
    parser.add_argument('--id', action='store', type=int, default=8000)
    parser.add_argument('--numbers', action='store', type=int, default=200)
    ...
    if args.sample_name=='all':
        obj_batch = os.listdir(args.dataset_path)
    else:
