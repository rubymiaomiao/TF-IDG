...
def test(obj_names, mvtec_path, checkpoint_path):
    ...
    for obj_name in obj_names:
        ...
        obj_image_f1_list.append(f1_max)

        obj_ap_pixel_list.append(ap_pixel)
        obj_auroc_pixel_list.append(auroc_pixel)
        obj_pixel_f1_list.append(max_f1_px)
        ...
        with open(f"{checkpoint_path}/result.csv", "a") as csvfile:
            writer = csv.writer(csvfile)
    ...
    print("F1 Image mean:  " + str(np.mean(obj_image_f1_list)))
    print("AUC Pixel mean:  " + str(np.mean(obj_auroc_pixel_list)))
    print("AP Pixel mean:  " + str(np.mean(obj_ap_pixel_list)))
    print("F1 Pixel mean:  " + str(np.mean(obj_pixel_f1_list)))
    print("Pro Pixel mean:  " + str(np.mean(obj_pro_pixel_list)))
    
    mean_data = [
    "Mean",
    round(100 * np.mean(obj_auroc_image_list), 1),
    round(100 * np.mean(obj_ap_image_list), 1),
    round(100 * np.mean(obj_image_f1_list), 1),
    round(100 * np.mean(obj_auroc_pixel_list), 1),
    round(100 * np.mean(obj_ap_pixel_list), 1),
    round(100 * np.mean(obj_pixel_f1_list), 1),
    round(100 * np.mean(obj_pro_pixel_list), 1)
    ]
...
if __name__=="__main__":
    ...
    if args.sample_name=='all':
        obj_list = os.listdir(args.dataset_path)
    else:
