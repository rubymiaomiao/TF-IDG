...
class ControlLDM(LatentDiffusion):
    ...
    def apply_model(self, x_noisy, t, cond, *args, **kwargs):
        ...
        cond_txt = torch.cat(cond['c_crossattn'], 1)#輸入進diffusion的embedding,將特定張量沿著指定維度進行拼接
        #print("x_noisy shape:", x_noisy.shape)#torch.Size([1, 4, 28, 28]) / torch.Size([1, 4, 64, 64])
        #print("t shape:", t.shape)#torch.Size([1, 257, 1024])
        #print("dino_embedding shape:", cond_txt.shape)#torch.Size([1, 257, 1024])
        if cond['c_concat'] is None:
            #无controlnet,纯粹dinov2加上diffusion
            eps = diffusion_model(x=x_noisy, timesteps=t, context=cond_txt, control=None, only_mid_control=self.only_mid_control)
        else:
            control = self.control_model(x=x_noisy, hint=torch.cat(cond['c_concat'], 1), timesteps=t, context=cond_txt)
            control = [c * scale for c, scale in zip(control, self.control_scales)]
            #Scaled control: [torch.Size([1, 320, 64, 64]), torch.Size([1, 320, 64, 64]), torch.Size([1, 320, 64, 64]), 
            # torch.Size([1, 320, 32, 32]), torch.Size([1, 640, 32, 32]), torch.Size([1, 640, 32, 32]), torch.Size([1, 640, 16, 16]), 
            # torch.Size([1, 1280, 16, 16]), torch.Size([1, 1280, 16, 16]), torch.Size([1, 1280, 8, 8]), torch.Size([1, 1280, 8, 8]), 
